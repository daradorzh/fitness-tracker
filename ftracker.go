...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	spentCaloriesRun := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return spentCaloriesRun
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedSm := meanSpeed(action, duration) * kmhInMsec
	meanSpeedPow := math.Pow(speedSm, 2)
	heightM := height / cmInM
	spentCaloriesWalking := ((walkingCaloriesWeightMultiplier*weight + (meanSpeedPow/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	meanSpeedSw := swimmingMeanSpeed(lengthPool, countPool, duration)
	spentCaloriesSwimming := (meanSpeedSw + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
